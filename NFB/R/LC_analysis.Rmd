...
sn.nor <- getSSN(tr.nor0, DV) # Normal sessions only
sn.inv <- getSSN(tr.inv0, DV) # Inverse trials only
sn.tra <- getSSN(tr.tra0, DV) # transfer trials only
...
Here we fit a linear regression model to Monotonic LCs, testing the interaction of protocol (TB, SMR) by sex. The first model reported is ANOVA; this is just included to illustrate similarity of results. Then a linear model is fitted to each training mode (normal, inverse, transfer) separately; and to the LC for all modes together. Finally tests are performed for monotonic LCs from session data and trial data separately.
...
Next we fit linear regression models to the Ideal LCs; again ANOVA is reported first for comparison. Each training mode is then tested separately, finishing with data from all modes together. 
