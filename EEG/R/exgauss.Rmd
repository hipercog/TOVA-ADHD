...
cbind(new_data, rtime = predict(mmu, newdata = new_data, re.form=NA)) %>%
  ggplot(aes(congruency, rtime, linetype=group)) + geom_point() + 
  geom_line(aes(group=group),size=1) +
  facet_wrap(~shape) + theme_bw() + ggtitle("mu values")

cbind(new_data, rtime = predict(msig, newdata = new_data, re.form=NA)) %>%
  ggplot(aes(congruency, rtime, linetype=group)) + geom_point() + 
  geom_line(aes(group=group),size=1) +
  facet_wrap(~shape) + theme_bw() + ggtitle("sigma values")

cbind(new_data, rtime = predict(mtau, newdata = new_data, re.form=NA)) %>%
  ggplot(aes(congruency, rtime, linetype=group)) + geom_point() + 
  geom_line(aes(group=group),size=1) +
  facet_wrap(~shape) + theme_bw() + ggtitle("tau values")

cbind(new_data, rtime = predict(mtau.log, newdata = new_data, re.form=NA)) %>%
  ggplot(aes(congruency, exp(rtime), linetype=group)) + geom_point() + 
  geom_line(aes(group=group),size=1) +
  facet_wrap(~shape) + theme_bw() + ggtitle("tau values (log model)")
