%% -Ben- Although it did not affect the functionality, output variable names were mixed up with respect to the help comments.
...
%% -Ben- Check what type of log we'll find - our non-pilot tests include more columns using Presentation's 'stimulus_properties' parameter
...
if ~salt
    convNames   = {'trial', 'Time', 'TTime', 'Uncertainty', 'Duration', 'Uncertainty', 'ReqTime', 'ReqDur'};%
else
    convNames   = {'trial', 'cond_num', 'vertices_num', 'Time', 'TTime', 'Uncertainty', 'Duration', 'Uncertainty', 'ReqTime', 'ReqDur'};%
end
...
%% Get variable names
%% -Ben- Have separated the functions for reading the header line and for normal lines, to handle whitespace.
...
%% -Ben- Columns whose names are duplicated get overwritten, i.e. 'Uncertainty'
...
try
    while( ischar(logLine) )
        ...
        for i = 1:NML
            ...
            if( k <= CNL )
                %% -Ben- Stop at the first completely empty line, since Pres always adds at least *some* column data
                if isempty(tmp)
                    tmp = 'NaN';
                end
...
%% SUB FUNCTIONS
%% -Ben- Renamed this but is otherwise the same, except for line 165 - now separators includes the last index, to buffer against the effect of calling diff()
...
%% -Ben- New version of sepEntries() simply ignores the presence of adjacent tabs, as these represent empty fields (which we want recorded as empty in the output
